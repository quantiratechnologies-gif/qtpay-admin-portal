...
export const CommissionMatrix: React.FC<CommissionMatrixProps> = ({
  ...
  const getStoredCurrentPassword = (email: string): string => {
    try {
  ...
  const saveAdminPassword = (email: string, pass: string) => {
    try {
      const saved = localStorage.getItem("qtpay_admin_credentials");
      const creds = saved
        ? JSON.parse(saved)
        : { "adminquantira@gmail.com": "admin123", "admin@qtpay.sa": "admin123" };
      creds[email.toLowerCase()] = pass;
      localStorage.setItem("qtpay_admin_credentials", JSON.stringify(creds));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const validatePasswordChange = (): string | null => {
    ...
    const currentExpectedPass = getStoredCurrentPassword(adminEmail);
    if (!currPass) return isAr ? "يرجى إدخال كلمة المرور الحالية" : "Current password is required";
    if (currPass !== currentExpectedPass)
    ...
    if (newPass.length < 8)
      return isAr ? "يجب أن تكون كلمة المرور 8 خانات على الأقل" : "Password must be at least 8 characters";
  ...
  const handleSaveAdminProfile = (e: React.FormEvent) => {
    ...
    const passToSave = newPass;
    setCurrPass("");
    setNewPass("");
    setConfirmPass("");

    if (changed) {
      saveAdminPassword(adminEmail, passToSave);
      setAuditLogs((prev) => [
        {
          id: `aud_${Date.now()}`,
